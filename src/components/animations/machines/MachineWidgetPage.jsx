/**
 * DcCoilMotorWidget.jsx  (수정판)
 *
 * 수정사항:
 *  1. 자석 두께 축소(1.2→0.7) + 위치 조정(3.4→2.4) → 코일이 자석에 가려지지 않음
 *  2. 코일 폭 W=2.2 → 자석 내면 사이 공간에 맞게
 *  3. 회전축 실린더 rotation={[Math.PI/2,0,0]} → Z축 방향으로 눕힘
 *  4. 자기장선 X 범위 = 자석 내면 좌표(INNER_X)로 수정
 *  5. 카메라 position=[0, 2.5, 8] → 코일 정면에서 보임
 *  6. Component import를 react에서 올바르게 처리
 *  7. GlbMotorModel에서 require() 제거 → 상단 import로 통일
 */
...
// ─── 자석 치수 상수 ───────────────────────────────────────────────────────
const MAGNET_X = 2.4; // 자석 중심 X 위치
const MAGNET_THICK = 0.7; // 자석 두께
const INNER_X = MAGNET_X - MAGNET_THICK / 2; // 내면 X ≈ 2.05

// ─── 코일 치수 ────────────────────────────────────────────────────────────
const W = 2.2; // 폭 (X) — 자석 내면 사이에 들어오도록
const H = 1.8; // 높이 (Y)
const R = 0.07; // 전선 반지름
...
function ProceduralMotor({ omegaRad, apiData }) {
  ...
  return (
    <group>
        ...
        [-0.55, 0, 0.55].map((y) => {
          ...
          return (
            <line key={`fl-${z}-${y}`}>
              ...
              <lineBasicMaterial color={DARK.field} transparent opacity={0.4} />
            </line>
          );
        }),
      )}
      ...
      <group ref={coilRef}>
        {/* 4변 전선 */}
        <Wire from={[-W / 2, H / 2, 0]} to={[W / 2, H / 2, 0]} /> {/* 상변 */}
        <Wire from={[-W / 2, -H / 2, 0]} to={[W / 2, -H / 2, 0]} /> {/* 하변 */}
        <Wire from={[-W / 2, -H / 2, 0]} to={[-W / 2, H / 2, 0]} /> {/* 좌변 */}
        <Wire from={[W / 2, -H / 2, 0]} to={[W / 2, H / 2, 0]} /> {/* 우변 */}
        {/* 코너 구 */}
        {[
          [-W / 2, H / 2],
          [W / 2, H / 2],
        ...
        {/* ★ 회전축 — Z 방향으로 눕힘 (rotation_axis: "z" 기본값) */}
...
export default function DcCoilMotorWidget({ apiData }) {
  ...
  return (
    ...
    >
      ...
      {/* ★ 3D 캔버스 — camera position 수정: [0, 2.5, 8] */}
